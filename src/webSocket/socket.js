    ...
    io.on("connection", (socket) => {
        ...
        */
        ...
        socket.on('checkInAppPurchase', async (arg) => {
            const userId = arg.user_id;
            const subscriptionType = arg.subscription_type;
            const userRoom = `User${arg.user_id}`;
            ...
            if (subscriptionType === 1) {
                ...
                if (findData[0] === undefined) {
                    ...
                    const firstData = findData[0];

                    const purchaseTime = parseFloat(firstData.purchaseTime);
                    const date = new Date(purchaseTime);
                    console.log("date", date);

                    const currentDate = new Date();
                    const futureDate = new Date(date.getTime());
                    futureDate.setMonth(futureDate.getMonth() + parseFloat(firstData.credit));
                    ...
                    if (remainingTimeMs >= 0) {
                        ...
                        remainingTime = `Remaining: ${remainingMonths} months, ${remainingDays} days, ${remainingHours} hours`;
                    } else {
                        remainingTime = "Remaining: 0 months, 0 days, 0 hours";
                    }

                    const response = {
                        validity: formatDateString(futureDate),
                ...
                if (findData[0] === undefined) {
                    ...
                    for (const checkCredit of findData) {
                        const response = {
                            ...
                            message: checkCredit.credit === 0 ? "Pack is over" : "Pack is running",
                            success: checkCredit.credit === 0 ? 0 : 1
                        };
